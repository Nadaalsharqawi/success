...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <div class="iq-card">
                <a href="{{ route('ads-Setting.create') }}" class="btn btn-text-primary font-weight-bold btn-fixed"
                ...
                <div class="iq-card-body">
                    ...
                    <table class="table table-striped table-bordered mt-4 table-hover text-center datatable-example"
                        ...
                        <tbody>
                            {{-- @dd($ads_settings) --}}
                            @foreach ($ads_settings as $index => $item)
                                <tr>
                                    <td>{{ $index + 1 }}</td>

                                    
                                   <td>{{ $item->countries->getNameAttribute() ?? "deleted" }}</td>
                                  
                                    <td>{{ $item->date_publication }}</td>
                                    <td>{{ $item->duration_status }}</td>
                                    <td class="text-center">
                                        <div class="flex align-items-center list-user-action">
                                            ...
                                            <div class="modal fade" id="delmodel{{ $item->id }}" tabindex="-1"
                                                role="dialog" aria-labelledby="delmodellabel"
                                                aria-hidden="true">
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title" id="delmodelLabel">
                                                                {{ __('admin.delete') }}</h5>
                                                        ...
                                                        <div class="modal-footer">
                                                            ...
                                                            {!! Form::open(['method' => 'delete', 'route' => ['ads-Setting.destroy', $item->id], 'class' => 'd-inline']) !!}
